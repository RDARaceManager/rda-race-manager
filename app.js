...
function render(){
  ...
  const podiumStage=(rows,championship=false)=>{if(!rows.length)return emptyState(championship?'Nessun podio campionato':'Nessun podio gara ufficiale');const order=[rows[1],rows[0],rows[2]].filter(Boolean);return order.map(r=>{const cls=r.pos==1?'winner':r.pos==2?'runner':'third';const medal=r.pos==1?'🥇':r.pos==2?'🥈':'🥉';const detail=championship?`${r.pts} pt • ${r.wins||0} vittorie`:`${r.car||'—'} • ${r.time||'—'}`;return `<div class="stage-driver ${cls}"><div class="laurel">${medal}<span>${r.pos}</span></div><div class="stage-step"><b>${r.name}</b><small>${detail}</small></div></div>`}).join('')};
  if($('#podiumDayFull')){$('#podiumDayTitle').textContent=pm.dayTitle||'Podio Gara';$('#podiumDayMeta').textContent=[pm.dayRace,pm.dayDate,pm.dayTrack].filter(Boolean).join(' • ');$('#podiumDayFull').innerHTML=podiumStage(pday,false)}
  const champArena=$('#podiumChampFull')?.closest('.champ-arena');if(D.championshipPodiumVisible&&pm.champClosed){if(champArena)champArena.style.display='block';$('#podiumChampTitle').textContent=pm.champName||'Podio Campionato';$('#podiumChampMeta').textContent=`MASTER ${pm.champMaster||'—'} • FINALE UFFICIALE`;$('#podiumChampFull').innerHTML=podiumStage(pchamp,true)}else if(champArena){champArena.style.display='none'};
